...
const ports = [
  ...
  { name: "ChatGPT Desktop", kind: "Desktop", description: "A unified theme for Chat, Work, and Codex with solid surfaces, clear diffs, and orange focus.", path: "agent_orange_dark.txt", format: "ChatGPT theme", href: "/api/themes/chatgpt", sourceHref: `${githubUrl}/blob/main/themes/chatgpt/agent_orange_dark.txt`, logo: "https://cdn.simpleicons.org/openai/e8e8e8", modes: true },
  { name: "Skill.md", kind: "Workflow", description: "A reusable guide for building Agent Orange UI, new ports, and aligned theme surfaces.", path: "SKILL.md", format: "Agent Orange skill", sourceHref: `${githubUrl}/blob/main/.agents/skills/agent-orange/SKILL.md`, logo: "", icon: Bot, modes: false, installCommand: "npx skills add edwardma33/agent-orange" },
];
    ...
    <main className="min-h-screen overflow-hidden bg-background text-foreground selection:bg-[var(--agent-orange-focus)] selection:text-primary-foreground">
      ...
      <section className="mx-auto grid max-w-7xl items-center gap-14 px-6 pb-24 pt-16 lg:grid-cols-[1.02fr_0.98fr] lg:px-8 lg:pb-32 lg:pt-24">
        <div className="max-w-2xl">
          <p className="mb-7 flex items-center gap-2 font-mono text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground"><span className="size-1.5 rounded-full bg-[var(--agent-orange-focus)]" /> Developer themes / 05 ports</p>
      ...
      <section id="ports" className="scroll-mt-6 border-t border-border bg-secondary">
        <div className="mx-auto max-w-7xl px-6 py-20 lg:px-8 lg:py-24">
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            {ports.map(({ name, kind, description, path, format, href, sourceHref, logo, icon: Icon, modes, installCommand }) => <article key={name} className="group rounded-lg border border-border bg-card p-5 transition-colors hover:border-[var(--agent-orange-focus)] sm:p-6"><div className="flex items-start justify-between"><div className="grid size-11 place-items-center rounded-md border border-border bg-background text-foreground group-hover:border-[var(--agent-orange-focus)]">{Icon ? <Icon className="size-5" aria-hidden="true" /> : <Image src={logo} alt="" aria-hidden="true" width={20} height={20} unoptimized className="size-5 invert dark:invert-0" />}</div><span className="rounded-full border border-border px-2.5 py-1 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">{kind}</span></div><h3 className="mt-8 font-heading text-2xl font-semibold tracking-[-0.035em] text-foreground">{name}</h3><p className="mt-2 min-h-18 text-sm leading-6 text-muted-foreground">{description}</p><div className="mt-7 border-t border-border pt-4"><p className="font-mono text-xs text-foreground">{path}</p><p className="mt-1 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">{format}</p>{installCommand ? <p className="mt-3 overflow-x-auto border border-border bg-background px-2 py-1.5 font-mono text-[10px] text-muted-foreground">{installCommand}</p> : null}<div className="mt-4 flex items-center justify-between"><a href={sourceHref} target="_blank" rel="noreferrer" className="inline-flex size-9 items-center justify-center rounded-md border border-border text-muted-foreground transition-colors hover:border-[var(--agent-orange-focus)] hover:text-[var(--agent-orange-focus-text)]" aria-label={`View ${name} source on GitHub`}><GitHubLogo className="size-4 invert dark:invert-0" /></a>{installCommand ? <InstallSkillButton command={installCommand} /> : modes ? <div className="flex gap-2"><a href={`${href}?mode=light`} className="inline-flex h-9 items-center gap-1.5 rounded-md border border-border px-2 text-[10px] font-semibold uppercase tracking-wide text-foreground transition-colors hover:border-[var(--agent-orange-focus)]" aria-label={`Download ${name} light theme`}><Download className="size-3.5" />Light</a><a href={`${href}?mode=dark`} className="inline-flex h-9 items-center gap-1.5 rounded-md bg-primary px-2 text-[10px] font-semibold uppercase tracking-wide text-primary-foreground transition-opacity hover:opacity-90" aria-label={`Download ${name} dark theme`}><Download className="size-3.5" />Dark</a></div> : null}</div></div></article>)}
          </div>
        </div>
      </section>
